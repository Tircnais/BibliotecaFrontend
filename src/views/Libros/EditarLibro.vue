...
<script>
    ...
    export default{
        data(){
            return{
                id: 0, intputTitulo: '', intputAnio: '', intputGenero: '', intputIdioma: '', intputDescp: '', url: 'http://localhost:8000/api/Libros/'
            }
        },
        mounted(){
        ...
        methods:{
            getLibro(){
                axios.get(this.url).then(
                    response => (
                        this.intputTitulo = response.data['titulo'],
                        this.intputAnio = response.data['anio'],
                        this.intputGenero = response.data['genero'],
                        this.intputIdioma = response.data['idioma'],
                        this.intputDescp = response.data['descripcion']
                    )
                );
            },
            actualizar(){
                ...
                if(this.intputTitulo.trim() === ''){
                    ...
                    enviarSolicitud('PUT', params, this.url, 'Libro actualizado');
                }
            }
        }
    }
